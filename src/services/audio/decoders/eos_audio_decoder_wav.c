...
static eos_result_t _wav_probe(const void *src, eos_audio_src_type_t src_type,
                               eos_audio_format_t *format)
{
    ...
    while (!found_fmt && offset < sizeof(wav_riff_header_t) + 4096)
    {
        ...
        {
            wav_fmt_chunk_t fmt;
            if (eos_fs_read(f, &fmt, sizeof(fmt)) != sizeof(fmt)) break;
    ...
    EOS_LOG_I("WAV probe: success (%dHz %dch %dbits)",
...
static eos_result_t _wav_open(eos_audio_decoder_dsc_t *dsc)
{
    ...
    wav_dec_data_t *wd = calloc(1, sizeof(wav_dec_data_t));
    if (!wd) return EOS_ERR_MEM;
    ...
    EOS_LOG_I("WAV decoder opened: %dHz %dch %dbits, %d samples",
              dsc->format.sample_rate, dsc->format.channels,
              dsc->format.bits_per_sample, dsc->format.total_samples);
    return EOS_OK;
}

static eos_result_t _wav_read(eos_audio_decoder_dsc_t *dsc,
                              void *buf, uint32_t buf_size, uint32_t *bytes_read)
{
    wav_dec_data_t *wd = (wav_dec_data_t *)dsc->user_data;
    if (!wd) return EOS_ERR_INVALID_STATE;
    ...
    if (ret < 0) return EOS_ERR_FILE_ERROR;
...
static eos_result_t _wav_seek(eos_audio_decoder_dsc_t *dsc, uint32_t sample)
{
    ...
    if (byte_offset >= wd->data_size) return EOS_ERR_INVALID_ARG;
